---
title: Introduction
description: "Get started with Minions Bundle: Content"
---

# Minions Bundle: Content

Content pipeline bundle — drafts, publications, channels, campaigns

## Overview

This project is part of the [Minions ecosystem](https://github.com/mxn2020/minions) and provides:

- **TypeScript SDK** (`@minions-bundles-content/sdk`) — Core library for Node.js
- **Python SDK** (`minions-bundles-content`) — Python bindings
- **CLI Tool** (`@minions-bundles-content/cli`) — Command-line interface
- **Web App** — Interactive playground at [app.minions.wtf](https://app.minions.wtf)

## Next Steps

- [Installation](/getting-started/installation/) — Set up the SDK
- [Quick Start](/getting-started/quick-start/) — Your first project
